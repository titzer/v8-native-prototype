...
TEST_F(MacroGenTest, Statements) {
  ...
  EXPECT_SIZE(7, WASM_IF_THEN(WASM_ZERO, WASM_NOP, WASM_NOP));

  EXPECT_SIZE(2, WASM_NOP);

  EXPECT_SIZE(4, WASM_BLOCK(1, WASM_NOP));
  EXPECT_SIZE(6, WASM_BLOCK(2, WASM_NOP, WASM_NOP));
  EXPECT_SIZE(8, WASM_BLOCK(3, WASM_NOP, WASM_NOP, WASM_NOP));

  EXPECT_SIZE(4, WASM_LOOP(1, WASM_NOP));
  EXPECT_SIZE(6, WASM_LOOP(2, WASM_NOP, WASM_NOP));
  EXPECT_SIZE(8, WASM_LOOP(3, WASM_NOP, WASM_NOP, WASM_NOP));
  EXPECT_SIZE(4, WASM_LOOP(1, WASM_BREAK(0)));
  EXPECT_SIZE(4, WASM_LOOP(1, WASM_CONTINUE(0)));

  EXPECT_SIZE(4, WASM_RETURN(1, WASM_ZERO));
  EXPECT_SIZE(6, WASM_RETURN(2, WASM_ZERO, WASM_ONE));
  EXPECT_SIZE(8, WASM_RETURN(3, WASM_ZERO, WASM_ONE, WASM_ZERO));

  EXPECT_SIZE(4, WASM_SWITCH(0, WASM_ZERO));
  EXPECT_SIZE(6, WASM_SWITCH(1, WASM_ZERO, WASM_NOP));
  EXPECT_SIZE(8, WASM_SWITCH(2, WASM_ZERO, WASM_NOP, WASM_NOP));

  EXPECT_SIZE(4, WASM_SWITCH_NF(0, WASM_ZERO));
  EXPECT_SIZE(6, WASM_SWITCH_NF(1, WASM_ZERO, WASM_NOP));
  EXPECT_SIZE(8, WASM_SWITCH_NF(2, WASM_ZERO, WASM_NOP, WASM_NOP));
}


TEST_F(MacroGenTest, MacroStatements) {
  EXPECT_SIZE(10, WASM_WHILE(WASM_INT8(0), WASM_NOP));
}
